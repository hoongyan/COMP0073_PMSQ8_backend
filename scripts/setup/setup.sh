...
docker exec comp0073_production_pgvector psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "CREATE EXTENSION IF NOT EXISTS vector;"
if [ $? -eq 0 ]; then
...
docker exec comp0073_production_pgvector psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT * FROM pg_extension WHERE extname = 'vector';" | grep -q "vector" && {
