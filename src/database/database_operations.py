...
import json  # Added for json.loads in retrieve_strategies

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))
from config.settings import get_settings
from config.logging_config import setup_logger
from src.models.data_model import Strategies, ScamReports, Base  # Added imports for __main__
...
# Module-level instance for efficiency (created at app startup)
...
class CRUDOperations:
    ...
    def create(self, db: Session, data: Dict[str, Any]) -> Optional[Any]:
    ...
    def create_bulk(self, db: Session, df: pd.DataFrame) -> int:
    ...
    def read(self, db: Session, record_id: Union[str, int]) -> Optional[Any]:
    ...
    def read_all(self, db: Session, limit: int = 100, offset: int = 0) -> List[Any]:
        """Read all records."""
        try:
    ...
    def update(self, db: Session, record_id: Union[str, int], data: Dict[str, Any]) -> Optional[Any]:
    ...
    def update_embedding(self, db: Session, record_id: Union[str, int], embedding: List[float], column_name: str = "embedding") -> bool:
    ...
    def update_embedding_bulk(self, db: Session, embeddings: Dict[Union[str, int], List[float]], column_name: str = "embedding") -> int:
...
# New subclass for strategy-specific operations (modular, avoids polluting generic CRUD)
class StrategiesCRUD(CRUDOperations):
    def __init__(self):
    ...
    def strategy_search(
        ...
        try:
            kb_size = db.query(Strategies).count()
            if kb_size == 0:
                self.logger.warning("No strategies in KB")
                return pd.DataFrame()
                
            # Start query
            query = db.query(Strategies)
            ...
            for dim in ['tech_literacy', 'language_proficiency', 'emotional_state']:
                if dim in user_profile and 'level' in user_profile[dim]:
                    ...
                    query = query.filter(
                        func.jsonb_extract_path_text(Strategies.user_profile, dim, 'level') == level
                    )
                
            # Apply metadata_filter if any
            if metadata_filter:
                for key, value in metadata_filter.items():
                    query = query.filter(getattr(Strategies, key) == value)
                
            # Rank by success_score DESC, limit
            query = query.order_by(Strategies.success_score.desc()).limit(limit)
            results = query.all()
            ...
            for strategy in results:
                db.execute(update(Strategies).where(Strategies.strategy_id == strategy.strategy_id).values(retrieval_count=Strategies.retrieval_count + 1))
                
            records = []
            for strat in results:
                record_data = {c.name: getattr(strat, c.name) for c in Strategies.__table__.columns}
    ...
    def retrieve_strategies(self, db: Session, user_profile: Union[str, Dict], top_k: int = 5, metadata_filter: Optional[Dict] = None) -> List[Dict]:
        """Retrieve strategy types using structured search. Returns list of dicts; no distances."""
        try:
            ...
            df = self.strategy_search(db, user_profile=user_profile_dict, limit=top_k, metadata_filter=metadata_filter)
            results = []
            if not df.empty:
                for record in df.to_dict(orient="records"):
                    result = {"strategy_type": record.get("strategy_type")}
    ...
    def prune_strategies(self, db: Session, min_kb_size: int = 50) -> int:
        ...
        try:
            ...
            deleted = db.query(Strategies).filter(
                Strategies.success_score < 0.6,
                Strategies.retrieval_count < 5
            ).delete()
            db.commit()
...
if __name__ == "__main__":
    ...
    # For testing, manually get a session (since not in FastAPI)
    ...
    # Test single create
    created_record = crud.create(db, sample_data)
    if created_record:
        print(f"Created record: {created_record.report_id}")
        test_id = created_record.report_id  # Use generated int ID
    else:
    ...
    # Test bulk create
    bulk_count = crud.create_bulk(db, bulk_data)
    print(f"Bulk created {bulk_count} records")

    # Assuming bulk creates IDs starting from next available, e.g., test_id + 1 and +2
    # For real testing, query to get IDs
...
#         with self.session_factory() as db:
#             try:
#                 kb_size = db.query(Strategies).count() #ensure kb size is large enough
#                 if kb_size < min_kb_size:
#                     self.logger.info(f"Skipping prune: KB size {kb_size} < {min_kb_size}")
#                     return 0
